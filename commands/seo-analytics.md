...
3. Run, in parallel where possible:
   - `npx tsx "${CLAUDE_PLUGIN_ROOT}/scripts/ga4.ts" traffic --days 28 --prev` (overall, period-over-period)
   - `npx tsx "${CLAUDE_PLUGIN_ROOT}/scripts/ga4.ts" traffic --organic --days 28` (organic only)
   - `npx tsx "${CLAUDE_PLUGIN_ROOT}/scripts/ga4.ts" pages --days 28 --limit 15`
   - `npx tsx "${CLAUDE_PLUGIN_ROOT}/scripts/gsc.ts" queries --days 28 --limit 20`
   - `npx tsx "${CLAUDE_PLUGIN_ROOT}/scripts/gsc.ts" pages --days 28 --limit 15`
